 ...
 * Supports Gemini 2.5 Flash with automatic fallback to Gemini 2.0 Flash and 1.5 Flash.
 */
...
export async function testGeminiApiKey(apiKey, model = 'gemini-2.5-flash') {
    ...
    const testModels = [model, 'gemini-2.0-flash', 'gemini-1.5-flash'];
    ...
    for (const m of testModels) {
        try {
            ...
            if (response.ok) {
                return { success: true, modelUsed: m, message: `Koneksi berhasil menggunakan model ${m}!` };
            }

            const errorData = await response.json().catch(() => ({}));
            const errMsg = errorData.error?.message || response.statusText;

            // If 404, maybe this model is not found, try next in loop
            if (response.status === 404) {
                continue;
            }

            if (response.status === 400 || response.status === 403 || response.status === 401) {
                return { success: false, message: `API Key tidak valid atau ditolak: ${errMsg}` };
            }
...
`;
    ...
    // Attempt with requested model, fallback to 2.0 or 1.5 if 404
    const candidateModels = [model];
    if (model !== 'gemini-2.0-flash') candidateModels.push('gemini-2.0-flash');
    if (model !== 'gemini-1.5-flash') candidateModels.push('gemini-1.5-flash');
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    throw new Error('Kuota permintaan Gemini API terlampaui (Rate limit). Mohon tunggu beberapa detik.');
                }

                throw new Error(`Google AI Studio Error (${response.status}): ${errMsg}`);
            }
            ...
            // If it's a model not found, loop will continue
